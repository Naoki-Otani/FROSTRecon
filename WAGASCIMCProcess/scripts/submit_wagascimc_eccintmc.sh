...
# ========= path settings =========
NEUT_OUTPUT_DIR="/group/nu/ninja/work/otani/FROSTReconData/ECCintMC/0-Neut"

OUTPUT_ROOT_DIR="/group/nu/ninja/work/otani/FROSTReconData/ECCintMC/1-WagasciMC"
...
for neut_output_file in "$NEUT_OUTPUT_DIR"/genev.*.root; do
    ...
    out_file="$OUT_DIR/b2mc_eccint_${neut_output_base%.root}.out"
    log_file="$LOG_DIR/b2mc_eccint_${neut_output_base%.root}.log"
    output_root_file="$OUTPUT_ROOT_DIR/b2mc_eccint_${neut_output_base%.root}.root"
    output_gdml_file="$OUT_GDML_DIR/b2mc_eccint.gdml"
