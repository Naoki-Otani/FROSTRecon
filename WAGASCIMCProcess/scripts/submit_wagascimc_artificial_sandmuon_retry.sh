...
OUTPUT_ROOT_DIR="/group/nu/ninja/work/otani/FROSTReconData/Artificial_sandmuonMC_retry/1-WagasciMC"
    ...
    bsub -q "$QUEUE" -o "$out_file" -N \
        bash -lc "
            ...
            apptainer run --bind /hsm/nu/ --bind /group/nu/ --bind /home/nu/ \"$SIF\" \
                ...
                --frost-sand-muon-retry \
                --frost-sand-muon-min-dwg-hits 4 \
                --frost-sand-muon-min-pm-hits 10 \
                --frost-sand-muon-retry-max-attempts 0
            $remove_gdml_command
        "
done
