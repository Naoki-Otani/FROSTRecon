...
for seed_file in "$SEED_DIR"/sandmuonmc_seed_*.root; do
    ...
    suffix="${seed_base#sandmuonmc_seed_}"

    out_file="$OUT_DIR/sandmuonmc_vertex_${suffix%.root}.out"
    log_file="$LOG_DIR/sandmuonmc_vertex_${suffix%.root}.log"
    vertex_file="$VERTEX_DIR/sandmuonmc_vertex_$suffix"
