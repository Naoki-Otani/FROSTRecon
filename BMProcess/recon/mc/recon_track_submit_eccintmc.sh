...
# ========= path settings =========
VERTEX_DIR="/group/nu/ninja/work/otani/FROSTReconData/ECCintMC/2-recon/vertex"

TRACK_DIR="/group/nu/ninja/work/otani/FROSTReconData/ECCintMC/2-recon/track"
...
for vertex_file in "$VERTEX_DIR"/eccintmc_vertex_*.root; do
    ...
    suffix="${vertex_base#eccintmc_vertex_}"
    out_file="$OUT_DIR/eccintmc_track_${suffix%.root}.out"
    log_file="$LOG_DIR/eccintmc_track_${suffix%.root}.log"
    track_file="$TRACK_DIR/eccintmc_track_$suffix"
