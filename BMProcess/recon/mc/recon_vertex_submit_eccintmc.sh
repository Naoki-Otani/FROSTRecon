...
for seed_file in "$SEED_DIR"/eccintmc_seed_*.root; do
    ...
    suffix="${seed_base#eccintmc_seed_}"

    out_file="$OUT_DIR/eccintmc_vertex_${suffix%.root}.out"
    log_file="$LOG_DIR/eccintmc_vertex_${suffix%.root}.log"
    vertex_file="$VERTEX_DIR/eccintmc_vertex_$suffix"
